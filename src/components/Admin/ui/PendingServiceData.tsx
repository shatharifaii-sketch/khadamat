...
const PendingServiceData = ({ service, setServiceToAccept }: Props) => {
    ...
    <div className="relative" dir={lang === "ar" ? "rtl" : "ltr"}>
      <div className="flex flex-col gap-6 overflow-y-auto max-h-[60vh] pr-2 text-start">
        <h1 className="flex flex-col">
          ...
          <span className="px-4 text-2xl">{service.title}</span>
        </h1>
        ...
        <p className="flex flex-col">
          ...
          <span className="px-4">{service.price_range}</span>
        </p>
        <div
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="check">{t("table.pending_services_management.data.with_appointments")}</label>
          </div>
          <Switch
            id="with_appointments"
            dir="ltr"
            checked={service.with_appointments}
            disabled
          />
        </div>
        <p className="flex flex-col">
          ...
          <span className="px-4">
            {service.whatsapp_number
              ? typeof service.whatsapp_number === "string"
                ? service.whatsapp_number
                : `${service.whatsapp_number.countryCode} ${service.whatsapp_number.number}`
              : "اونلاين"}
          </span>
        </p>
        <p className="flex flex-col">
          ...
          <span className="px-4">
            {service.location
              ? t(service.location)
              : t("table.pending_services_management.data.offline_fallback")}
          </span>
        </p>
        <p className="flex flex-col">
        ...
        <div>
          <h3 className="text-lg font-semibold">
            {t("table.pending_services_management.data.attachments")}
          </h3>
      ...
      <div className="w-full">
        <h3 className="text-lg font-semibold">
          {t("table.pending_services_management.data.accept_title")}
        </h3>
