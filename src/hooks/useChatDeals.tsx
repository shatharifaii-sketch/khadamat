...
async function acceptDealWithId({
  dealId,
  role,
  isAccepted,
}: {
  ...
  if (!dealId) return { success: false, error: "missing_data", data: null };
  ...
  if (role == "client") {
    updateData = {
      client_accepted: !isAccepted,
      client_rejected: false,
    };
  } else if (role == "provider") {
    updateData = {
      provider_accepted: !isAccepted,
...
async function rejectDealWithId({
  dealId,
  role,
  isRejected,
}: {
  ...
  if (role == "client") {
    updateData = {
      client_rejected: !isRejected,
      client_accepted: false,
    };
  } else if (role == "provider") {
    updateData = {
      provider_rejected: !isRejected,
...
export const useChatDeals = ({
  ...
  const {
    ...
    mutationFn: acceptDealWithId,
    onSuccess: async ({ success, error, data }) => {
      if (error) {
        toast.error(t(error));
      }
      ...
      const { data: emailData, error: emailError } = await supabase.functions.invoke("accept-deal", {
        body: {
          values: {
            dealId: data.dealId,
      ...
      if (emailError) {
        console.error("error creating deal: ", emailError);
        return { success: false, error: emailError.message };
      }

      if (emailData.error && !emailData.success) {
  ...
  const {
    ...
    onSuccess: async ({ success, error, data }) => {
      ...
      const { data: emailData, error: emailError } = await supabase.functions.invoke("reject-deal", {
        body: {
          values: {
            dealId: data.dealId,
            updateData: data.updateData,
