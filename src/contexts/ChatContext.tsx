...
const updateMessageReadAt = async ({
  message,
  userId,
}: {
  message: Message | any;
...
export const ChatProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const replaceLocalMessage = (tempId: string, realMessage: any) => {
    setMessages((prev) => {
      ...
      console.log(
        "Messages before replace:",
        prev.map((m) => ({ id: m.id, pending: m.pending })),
      );
      const updated = prev.map((msg) =>
        msg.id === tempId ? realMessage : msg,
      );
      console.log(
        "Messages after replace:",
        updated.map((m) => ({ id: m.id, pending: m.pending })),
      );
      return updated;
    });
  };
  ...
  const sendMessage = async ({
    ...
    try {
      ...
      if (file && typeof file === "object") {
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from("messages-images").getPublicUrl(fileName);
      ...
      const { data: notif, error: notifError } =
        await supabase.functions.invoke("send-message-notification-email", {
          body: {
            conversation_id: activeConversation.id,
            sender_id: user.id,
            message: content,
            client_id: activeConversation.client_id,
            provider_id: activeConversation.provider_id,
            service_id: activeConversation.service_id,
          },
        });
      ...
      if (!notif.success && notif.error) {
        console.error("error occurred", notif.error);
        toast.error(t(notif.error ?? "unexpected_error_occured"));
      }
    } catch (error) {
      console.error("Error sending message:", error);
      toast.error("حدث خطاء في ارسال الرسالة");
